...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, isSaveCgmSource) {
    ...
    var tdd7 = (meal_data.TDDAIMI7) ? meal_data.TDDAIMI7 : (((basal * 12) * 100) / 21);
    ...
    var getISFforBG = function (bg) {
        ...
        var diff = sens_normalTarget - (sens_normalTarget / scaler);
        var result = sens_normalTarget - diff;
        if (!result) console.log("failed ISF for bg: ", round(bg, 2), "sens_normalTarget: ", round(sens_normalTarget, 2), "sens_BG: ", round(sens_BG, 2), "scaler: ", round(scaler, 2), "diff: ", round(diff, 2));
        return result;
    }
    variable_sens = getISFforBG(bg);
