...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, isSaveCgmSource) {
    ...
    //console.error("Calculated TDD: " + round(TDD, 2) + "; ");
    ...
    console.error("  Adjusted: " + round(TDD, 2));
    ...
    console.log("Circadian_sensitivity factor : " + round(circadian_sensitivity, 2) + "; ");
    ...
    if (high_temptarget_raises_sensitivity && profile.temptargetSet && target_bg > normalTarget || profile.low_temptarget_lowers_sensitivity && profile.temptargetSet && target_bg < normalTarget) {
        ...
        console.log("Sensitivity ratio set to " + round(sensitivityRatio, 2) + " based on temp target of " + target_bg + "; ");
        sens = sens / sensitivityRatio;
        sens = round(sens, 1);
        console.log("ISF from " + round(variable_sens, 2) + " to " + round(sens, 2) + " due to temp target; ");
    }
    else {
        ...
        if (sensitivityRatio > 1) {
            sensitivityRatio = Math.min(sensitivityRatio, profile.autosens_max);
            sensitivityRatio = round(sensitivityRatio, 2);
            console.log("Sensitivity ratio: " + round(sensitivityRatio, 2) + "; ");
        }
    ...
    if (sensitivityRatio && profile.openapsama_useautosens === true) {
        ...
        if (basal !== profile_current_basal) {
            console.log("Adjusting basal from " + round(profile_current_basal, 2) + " to " + round(basal, 2) + "; ");
        } else {
            console.log("Basal unchanged: " + round(basal, 2) + "; ");
        }
    }
    ...
    if (profile.temptargetSet) {
        ...
        if (profile.sensitivity_raises_target && sensitivityRatio < 1 && profile.openapsama_useautosens === true || profile.resistance_lowers_target && sensitivityRatio > 1 && profile.openapsama_useautosens === true) {
            ...
            if (target_bg === new_target_bg) {
                console.log("target_bg unchanged: " + round(new_target_bg, 2) + "; ");
            } else {
                console.log("target_bg from " + target_bg + " to " + round(new_target_bg, 2) + "; ");
            }
    ...
    var REBG = EBG / min_bg;
    console.log("Experimental test, EBG : " + round(EBG, 2) + " REBG : " + round(REBG, 2) + " ; ");
    ...
    if (!profile.temptargetSet && HypoPredBG <= 125 && profile.sensitivity_raises_target) {//&& glucose_status.delta <= 0
        ...
        basal = round_basal(basal, profile);
        if (basal !== profile_current_basal) {
            console.log("Adjusting basal from " + profile_current_basal + " to " + round(basal, 2) + "; ");
        } else {
            console.log("Basal unchanged: " + round(basal, 2) + "; ");
        }
    }
    ...
    if (meal_data.mealCOB > 0 && delta_accl > 0) {
        future_sens = (1800 / (Math.log((((eventualBG * 0.75) + (bg * 0.25)) / ins_val) + 1) * TDD));
        console.log("Future state sensitivity is " + round(future_sens, 2) + " weighted on eventual BG due to COB");
        rT.reason += "Dosing sensitivity: " + round(future_sens, 2) + " weighted on predicted BG due to COB;";
    }
    else if (glucose_status.delta > 4 && delta_accl > 10 && bg < 180 && eventualBG > bg) {
        future_sens = (1800 / (Math.log((((eventualBG * 0.5) + (bg * 0.5)) / ins_val) + 1) * TDD));
        console.log("Future state sensitivity is " + round(future_sens, 2) + " weighted on predicted bg due to increasing deltas");
        rT.reason += "Dosing sensitivity: " + round(future_sens, 2) + " weighted on predicted BG due to delta;";
    }
    ...
    /*else if( glucose_status.delta > 0 && delta_accl > 0 && bg > 198 || eventualBG > bg && bg >
     198) {
        ...
        console.log("Future state sensitivity is " + round(future_sens, 2) + " based on current bg due to +ve delta");
    }
    else {
        future_sens = (1800 / (Math.log((Math.max(minPredBG, 1) / ins_val) + 1) * TDD));
        console.log("Future state sensitivity is " + round(future_sens, 2) + " based on min predited bg due to -ve delta");
        rT.reason += "Dosing sensitivity: " + round(future_sens, 2) + " using eventual BG;";
    }
    //future_sens = future_sens * circadian_sensitivity;

    future_sens = round(future_sens, 1);

    console.log("Future sens adjusted to : " + round(future_sens, 2) + "; ");
    ...
    console.log("minPredBG: " + convert_bg(minPredBG, profile) + " minIOBPredBG: " + convert_bg(minIOBPredBG, profile) + " minZTGuardBG: " + convert_bg(minZTGuardBG, profile));
    if (minCOBPredBG < 999) {
        console.log(" minCOBPredBG: " + convert_bg(minCOBPredBG, profile));
    }
    if (minUAMPredBG < 999) {
        console.log(" minUAMPredBG: " + convert_bg(minUAMPredBG, profile));
    }
    console.error(" avgPredBG:", convert_bg(avgPredBG, profile), "COB:", meal_data.mealCOB, "/", meal_data.carbs);
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            console.error("UAM Boost 1 value is " + round(uamBoost1, 4) + "; ");
            ...
            console.error("UAM Boost 2 value is " + round(uamBoost2, 4) + "; ");
            rT.reason += ("UAM Boost 1 value is " + round(uamBoost1, 4) + "; ");
            rT.reason += ("UAM Boost 2 value is " + round(uamBoost2, 4) + "; ");
            ...
            console.error("Base boost insulin is " + round(boostInsulinReq, 4) + " iu; ");
            console.error("            ");

            //cARB HANDLING INSULIN UPTICK CODE.
            //With COB, allow a large initial bolus
            if (boost_time && COB > 0 && lastCarbAge < 15) {
            ...
            else if (boost_time && COB > 0 && lastCarbAge < 40 && glucose_status.delta > 5) {
                var cob_boost_max = Math.max(((COB / CR) / insulinReqPCT), boost_max);
            ...
            else if (glucose_status.delta >= 5 && glucose_status.short_avgdelta >= 3 && uamBoost1 > 1.2 && uamBoost2 > 2 && boost_time && iob_data.iob < boostMaxIOB && boost_scale < 3 && eventualBG > target_bg && bg > 80 && insulinReq > 0) {
            ...
            else if (bg > 108 && bg < 181 && glucose_status.delta > 3 && delta_accl > 0 && eventualBG > target_bg && iob_data.iob < boostMaxIOB && boost_time && profile.enableBoostPercentScale === true) {
                if (insulinReq > boostMaxIOB - iob_data.iob) {
                    insulinReq = boostMaxIOB - iob_data.iob;
                }
                else {
            ...
            else if (boost_time && glucose_status.delta > 0 && delta_accl >= 0) {
                var microBolus = Math.floor(Math.min(insulinReq / insulinReqPCT, boost_max) * roundSMBTo) / roundSMBTo;
