...
class AIMIPlugin @Inject constructor(
    ...
    override fun invoke(initiator: String, tempBasalFallback: Boolean) {
        ...
        var minBg = hardLimits.verifyHardLimits(Round.roundTo(profile.getTargetLowMgdl(), 0.1), R.string.profile_low_target, HardLimits.VERY_HARD_LIMIT_MIN_BG_AIMI[0], HardLimits.VERY_HARD_LIMIT_MIN_BG_AIMI[1])
        var maxBg = hardLimits.verifyHardLimits(Round.roundTo(profile.getTargetHighMgdl(), 0.1), R.string.profile_high_target, HardLimits.VERY_HARD_LIMIT_MAX_BG_AIMI[0], HardLimits.VERY_HARD_LIMIT_MAX_BG_AIMI[1])
        var targetBg = hardLimits.verifyHardLimits(profile.getTargetMgdl(), R.string.temp_target_value, HardLimits.VERY_HARD_LIMIT_TARGET_BG_AIMI[0], HardLimits.VERY_HARD_LIMIT_TARGET_BG_AIMI[1])
        ...
        if (!hardLimits.checkHardLimits(profile.dia, R.string.profile_dia, hardLimits.minDiaAimi(), hardLimits.maxDia())) return
