...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, isSaveCgmSource) {
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            else IOBpredBG = IOBpredBGs[IOBpredBGs.length-1] + (round(( -iobTick.activity * getISFforBG(Math.max( IOBpredBGs[IOBpredBGs.length-1],39)) * 5 ),2)) + predDev;

            // calculate predBGs with long zero temp without deviations
            var ZTpredBG = (!TDD) var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + predZTBGI;
            else var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + (round(( -iobTick.iobWithZeroTemp.activity * getISFforBG(Math.max(ZTpredBGs[ZTpredBGs.length-1],39)) * 5 ), 2));
            ...
            else UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + (round(( -iobTick.activity * getISFforBG(Math.max(UAMpredBGs[UAMpredBGs.length-1],39)) * 5 ),2)) + Math.min(0, predDev) + predUCI;
